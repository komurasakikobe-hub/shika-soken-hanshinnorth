...
各院に以下を付与し、q_excluded=True を全ページで除外する。
  q_in_osaka : 住所に対象5市町（伊丹市/宝塚市/川西市/三田市/猪名川町）のいずれかを含む
...
def main():
    ...
    for pid, c in items:
        AREA_CITIES = ("伊丹市", "宝塚市", "川西市", "三田市", "猪名川町")
        in_osaka = any(city in (c.get("address") or "") for city in AREA_CITIES)
